...
public class BroadbandHandler implements Route {
   ...
   * the state, county, broadband access percentage, and date and time of the request. If any errors
   * occur during data fetching or if parameters are missing, an appropriate error response is
   ...
   * @return A serialized JSON string representing either a success or failure response. The success
   *     response includes fetched data, while the failure response includes an error message
  ...
  /**
   * Represents a successful fetch operation, containing the response type and a map of response
   * data.
   *
  ...
  public record FetchSuccessResponse(String response_type, Map<String, Object> responseMap) {
    ...
    /**
     * Constructor for creating a FetchSuccessResponse with a specified map of response data. Sets
     * the response type to "success".
     *
  ...
  /**
   * Represents a failure response for fetch operations containing a response type indicating the
   * nature of the failure and the default response type of "error".
   *
  ...
  public record FetchFailureResponse(String response_type) {
    ...
    /**
     * Converts the response into a JSON string indicating an error response using the Moshi
     * library.
     *
